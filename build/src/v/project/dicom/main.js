...
({
  ...
  v.style.load(
    v.css.materialSymbolsOutlined_css,
    targetElement
  );
  ...
  return v.dom
    .loadScripts(
      [
        '/src/assets/dicom/dcmjs.js',
        '/src/assets/dicom/dcmjs-imaging.min.js',
      ],
      () => {},
      targetElement
    )
    .then(async () => {
      ...
      const { StandardColorPalette } =
        window.dcmjsImaging.constants;
      const initOpts = {
        webAssemblyModulePathOrUrl:
          '/src/assets/dicom/native-pixel-decoder.wasm',
      ...
      const onPointerMove = (e) => {
        if (locals.selectedBox?.canvasContainer) {
          if (
            locals.toolName === 'none' &&
            locals.selectedBox?.lines
          ) {
            let rect =
              locals.selectedBox.overlayCanvas.getBoundingClientRect();
            let xp = e.clientX - rect.left;
            let yp = e.clientY - rect.top;
            let line;
            let dmin = 26;
            for (l of locals.selectedBox?.lines) {
              // let dx1 = xp - l.x1;
              // let dy1 = yp - l.y1;
              // let dx2 = l.x2 - l.x1;
              // let dy2 = l.y2 - l.y1;
          ...
          if (
            ...
            if (locals.toolName === 'cine') {
              if (
                ...
                locals.selectedBox.target.getNumberOfFrames() >
                  1
              ) {
                const diff =
                  Math.abs(diffX) > Math.abs(diffY)
                    ...
                    : diffY;
                const next =
                  Math.max(1, 0 | Math.abs(diff / 5)) *
                  Math.sign(diff);
                if (
                  locals.selectedBox.renderOptions
                    .frameNumber +
                    next <=
                    locals.selectedBox.target.getNumberOfFrames() -
                      1 &&
              ...
              if (
                locals.selectedBox.renderOptions.windowLevel
              ) {
                let ww =
                ...
                if (
                  ww +
                    locals.selectedBox.ww * 0.001 * diffX <=
                  1
                ) {
                  return;
                }
                ww = Math.round(
                  ww + locals.selectedBox.ww * 0.001 * diffX
                );
                wl = Math.round(
                  wl - locals.selectedBox.ww * 0.001 * diffY
                );
                locals.selectedBox.renderOptions.windowLevel.setWindow(
                  ww
                );
                locals.selectedBox.renderOptions.windowLevel.setLevel(
                  wl
                );
                if (
                  !locals.selectedBox.renderingResult
                    .windowLevel
                ) {
                  if (wl >= locals.selectedBox.wl) {
                    locals.selectedBox.contrast =
                      ((locals.selectedBox.ww /
                        locals.selectedBox.wl) *
                        wl) /
                      ww;
              ...
              if (locals.pointer.windowing === 1) {
                ...
                locals.selectedBox.measurements.push(
                  measure
                );
                locals.pointer.windowing = 2;
              } else if (
                locals.selectedBox.measurements.length
              ) {
                let measure =
      ...
      document.addEventListener('pointerup', (e) => {
        if (locals.pointer.windowing) {
          let dx =
            e.clientX - locals.pointer.origin.clientX;
          let dy =
            e.clientY - locals.pointer.origin.clientY;
          let d2 = dx * dx + dy * dy;
          if (locals.toolName.startsWith('measure_')) {
            if (locals.toolName !== 'measure_angle') {
              if (
                (d2 > 9 &&
                  locals.pointer.windowing === 2) ||
                locals.pointer.windowing > 2
        ...
        if (opts.imageBitmap) {
          ...
          try {
            ...
            if (
              ...
              opts.image.renderedPixels[opts.frame] =
                new Uint8Array(
                  opts.image.renderingResult[
                    opts.frame
                  ].result.pixels
            ...
            if (opts.useWebGl) {
              const gl =
                opts.canvasElement.getContext('webgl');
              gl.viewport(
                0,
                0,
              ...
              const ctx =
                opts.canvasElement.getContext('2d');
              ctx.clearRect(
                0,
                0,
                opts.canvasElement.width,
      ...
      const updateFrame = (box) =>
        new Promise((resolve) => {
          if (!box.renderOptions) resolve();
          else {
            if (box.busyRenderingFrame)
              box.needsFrameRefresh = true;
            else {
              box.busyRenderingFrame = true;
              requestAnimationFrame(() => {
                box.target.updateRenderOptions(
                  box.renderOptions
                );
                box.renderingResult = renderFrame(
                  box.renderOptions
                );
                updateOverlays(box);
                box.renderOptions.frame =
                  box.renderingResult.frame;
      ...
      const selectImage = async (fid, box) => {
        ...
        box.then(async () => {
          ...
          box.target = target;
          let savedData =
            localStorage[target.fid] &&
            JSON.parse(localStorage[target.fid]);
          box.frameInterval =
            savedData?.frameInterval ??
            1000 / (target.FrameTime || 33);
          box.offsetX = savedData?.offsetX ?? 0;
          box.offsetY = savedData?.offsetY ?? 0;
          box.scale = savedData?.scale ?? 1;
          box.baseScale = savedData?.baseScale ?? 1;
          box.transforms = savedData?.transforms ?? [];
          box.measurements = savedData?.measurements ?? [];
          box.ww =
            savedData?.ww ??
          ...
          box.wl =
            ...
            128;
          box.contrast = savedData?.contrast ?? 1;
          box.brightness = savedData?.brightness ?? 1;
          box.inverted = savedData?.inverted ?? false;
          box.savedWindowLevel = savedData?.windowLevel;
          locals.navbar.invert.checked = box.inverted;
          let newCanvasContainer =
            await v.project.dicom.canvas({
              box,
              pointer: locals.pointer,
              updateFrame,
            });
          box.canvasElement =
            newCanvasContainer.canvasElement;
          if (box.canvasContainer)
            box.canvasContainer.replaceWith(
              newCanvasContainer
            );
          else
            box.container.replaceChildren(
              newCanvasContainer
            );
          ...
          if (target.imageBitmap)
            box.renderOptions = {
              ...
              frame: savedData?.frame ?? 0,
              frameNumber: savedData?.frameNumber ?? 0,
              windowLevel: savedData?.windowLevel
                ? new WindowLevel(
                    savedData.windowLevel.window,
                    savedData.windowLevel.level
                  )
                : undefined,
            };
          else
      ...
      const rescaleCanvas = (box) => {
        if (box.target && box.canvasContainer) {
          box.baseScale = Math.min(
            box.result.clientWidth /
              box.canvasElement.width,
            box.result.clientHeight /
              box.canvasElement.height
          );
          box.canvasContainer.style.scale = (
            box.scale * box.baseScale
          ).toFixed(3);
          updateFrame(box);
        }
      };
      const selectBox = (box) => {
        if (locals.selectedBox)
          locals.selectedBox.result.classList.remove(
            'border-white'
          );
      ...
      const loadFileAlt = (source) =>
        new Promise((resolve) => {
          ...
          var xml = FS.readFile(xmlFilePath, {
            encoding: 'utf8',
          });
          var header = new DOMParser().parseFromString(
            xml,
            'text/xml'
          );
          console.log(header);
          ...
          const samplesPerPixel =
            image.elements.SamplesPerPixel.value || 1;
          ...
          var offset =
            stat.size -
            canvasHeight * canvasWidth * samplesPerPixel;
      ...
      const loadFile = (file) =>
        new Promise(async (resolve) => {
          ...
          if (target.fid in locals.loadedFiles) {
            ...
            locals.loadingPercent = 0;
            locals.loadingTimer = setInterval(() => {
              locals.loadingPercent +=
                (1 - locals.loadingPercent) * 0.005;
              locals.loadingBar.style.width = `${
                locals.loadingPercent * 100
              }%`;
            }, 1);
            setTimeout(async () => {
              if (file.fromServer)
                file.chunks = await fetch(
                  '/instance/read-file',
                  {
                    method: 'POST',
                    headers: {
              ...
              clearInterval(locals.loadingTimer);
              locals.loadingBar.style.width = '100%';
              locals.loadedFiles[target.fid] = target;
              if (file.type.startsWith('image/')) {
                try {
                  target.sid = `-image-sid-${
                    Date.now() + locals.counter++
                  }`;
                  target.rid = 1;
                ...
                const loadDicomImage = (source) => {
                  ...
                  target.image.PixelSpacing ||= target.image
                    .elements?.PixelSpacing || [
                  ...
                  target.image.WindowCenter ||=
                    ...
                    128;
                  target.PatientID =
                    target.image.elements?.PatientID;
                  ...
                  target.InstanceNumber =
                    target.image.elements?.InstanceNumber;
                  target.StudyID =
                    target.image.elements?.StudyID;
                  ...
                  target.SeriesNumber =
                    target.image.elements?.SeriesNumber;
                  target.PatientBirthDate =
                    target.image.elements?.PatientBirthDate;
                  target.Modality =
                    target.image.elements?.Modality;
                  target.ImageComments =
                    target.image.elements?.ImageComments;
                  target.FrameTime =
                    target.image.elements?.FrameTime;
                  target.PatientSex =
                    target.image.elements?.PatientSex;
                  target.Rows = target.image.elements?.Rows;
                  target.Columns =
                    target.image.elements?.Columns;
                  target.SeriesDescription =
                    target.image.elements?.SeriesDescription;
                  target.StudyDate =
                  ...
                  target.StudyID =
                    target.image.elements?.StudyID;
                  target.SOPClassUID =
                    target.image.elements?.SOPClassUID;
                  target.SOPInstanceUID =
                    target.image.elements?.SOPInstanceUID;
                  ...
                  target.ImageType =
                    target.image.elements?.ImageType;
                  target.PixelSpacing ||=
                    target.image.PixelSpacing;
                  target.PixelSpacingUnit = target.image
                    .elements?.PixelSpacing
                    ? 'cm'
                    : target.image.elements
                  ...
                  target.ProtocolName =
                    target.image.elements?.ProtocolName;
                ...
                try {
                  ...
                  try {
                    await v.dom
                      .loadScripts(
                        [
                          '/src/assets/dicom/old/dcmjs.js',
                          '/src/assets/dicom/old/pace.js',
                        ],
                        () => {},
                        targetElement
                      )
                      .then(() => {
                        loadFileAlt(source).then(
                          (result) => {
                            if (result) {
                              target.imageBitmap =
                                result.imageBitmap;
                              ...
                              target.Rows =
                                target.image.elements?.Rows?.value;
                              target.Columns =
      ...
      const uploadFiles = (files) => {
        locals.loadingFiles = files;
        locals.loading.classList.remove('d-none');
        files
          .reduce(
            (p, c, i) =>
              p.then(() => {
                locals.loadingTitle.textContent = `${
                  i + 1
                } / ${locals.loadingFiles.length}`;
                return loadFile(c);
              }),
            Promise.resolve()
          )
          .then(() => {
            locals.loading.classList.add('d-none');
            if (Object.keys(locals.archive).length)
              locals.sidebar.reloadList(
                locals.archive,
                renderFrame,
      ...
      const saveData = () => {
        console.log(locals.selectedBox);
        if (locals.selectedBox?.target)
          localStorage[locals.selectedBox.target.fid] =
            JSON.stringify({
              brightness: locals.selectedBox.brightness,
              contrast: locals.selectedBox.contrast,
              frameInterval:
                locals.selectedBox.frameInterval,
              inverted: locals.selectedBox.inverted,
              measurements: locals.selectedBox.measurements,
              frame: locals.selectedBox.renderOptions.frame,
              frameNumber:
                locals.selectedBox.renderOptions
                  .frameNumber,
              windowLevel: {
              ...
              baseScale: locals.selectedBox.baseScale,
              scale: locals.selectedBox.scale,
              transforms: locals.selectedBox.transforms,
              wl: locals.selectedBox.wl,
              ww: locals.selectedBox.ww,
              offsetX: locals.selectedBox.offsetX,
              offsetY: locals.selectedBox.offsetY,
            });
      };
      const newBox = () =>
        v.project.dicom
          .box({
            close: (box) => {
              box.remove();
              locals.boxes.splice(
                locals.boxes.indexOf(box._owner_),
                1
              );
      ...
      const rearrangeBoxes = (r, c) => {
        ...
        locals.rearrangeBoxesTimeout = setTimeout(() => {
          if (locals.boxes) {
            ...
            if (!rows) {
              rows = Math.floor(
                Math.sqrt(locals.boxes.length)
              );
              if (locals.boxes.length / rows - rows > 1)
                rows++;
              cols = Math.ceil(locals.boxes.length / rows);
              if (
                locals.body.offsetHeight >
                locals.body.offsetWidth
              ) {
                rows += cols;
                cols = rows - cols;
                rows -= cols;
              ...
              Array.from({ length: rows }).map((_, m) =>
                v.div({
                  ...
                  content: Array.from({
                    length: Math.min(
                      cols,
                      locals.boxes.length - m * cols
                    ),
                  }).map((_, n) =>
                    v.div({
                      className:
                        'col p-0 position-relative',
      ...
      const showCursorIcon = () => {
        ...
        if (locals.cursorIcon.textContent === 'circle')
          ...
          locals.cursorIcon.style.removeProperty(
            'transform'
          );
      };
      ...
      const addTransform = (transform) => {
        locals.selectedBox.transforms.splice(
          0,
          0,
          transform
        );
      ...
      const updateOverlays = (box) => {
        let s = box.scale * box.baseScale;
        box.overlayCanvas.width =
          box.overlayCanvas.offsetWidth * 2;
        box.overlayCanvas.height =
          box.overlayCanvas.offsetHeight * 2;
        let orect =
          box.overlayCanvas.getBoundingClientRect();
        let crect =
          box.canvasElement.getBoundingClientRect();
        let offsetX =
          (crect.left + crect.right) * 0.5 - orect.left;
        let offsetY =
          (crect.top + crect.bottom) * 0.5 - orect.top;
        ...
        let measurements = box.measurements.filter(
          (m) =>
            m.frame === undefined ||
            m.frame === box.renderOptions.frameNumber
        );
        if (
          box.selectedLine &&
        ...
        for (let measure of measurements) {
          ...
          ctx.beginPath();
          if (
            measure.type === 'measure_length' ||
            measure.type === 'measure_angle'
          )
            for (let i = 1; i < ps.length; i++) {
              ctx.moveTo(
                ps[i - 1].x - ps[i].cn,
          ...
          else if (measure.type === 'measure_oval')
            ctx.ellipse(
              ps[0].x * 0.5 + ps[1].x * 0.5,
              ps[0].y * 0.5 + ps[1].y * 0.5,
              Math.abs(ps[0].x * 0.5 - ps[1].x * 0.5),
              Math.abs(ps[0].y * 0.5 - ps[1].y * 0.5),
              0,
              0,
              Math.PI * 2
            );
          else if (measure.type === 'measure_rect')
            ctx.rect(
              ps[0].x,
              ps[0].y,
              ps[1].x - ps[0].x,
      ...
      return v
        .main({
          ...
          content: [
            ...
            (locals.loading = await v
              .div({
                className: 'w-100 d-none',
                style:
                  'height:.6rem;display:flex;position:relative',
                content: [
                  v.div({
                    className: 'flex-grow-1 h-100',
                    content: (locals.loadingBar =
                      await v.div({
                        className:
                          'h-100 animated-gradient',
                      })),
                  }),
                  (locals.loadingTitle = await v.span({
                    style: {
                      position: 'absolute',
                      inset: '0',
                      display: 'flex',
                      ...
                      textShadow: `0 0 2px ${primaryColor}, 0 0 2px ${primaryColor}, 0 0 1px ${primaryColor}, 0 0 1px ${primaryColor}`,
                    },
                  })),
                ],
              })
              .useStyle('loading-gradient', {
                '.animated-gradient': {
                  background: `repeating-linear-gradient(-45deg, ${primaryColor} 0%, ${primaryColor} 25%, ${secondaryColor} 50%, ${primaryColor} 75%, ${primaryColor} 100%)`,
                  width: '100%',
            ...
            (locals.navbar = await v.project.dicom.navbar({
              ...
              setLinked: (value) => {
                if (locals.selectedBox.selectedLine) {
                  locals.selectedBox.selectedLine.measure.frame =
                    value
                      ? locals.selectedBox.renderOptions
                          .frameNumber
                      : undefined;
